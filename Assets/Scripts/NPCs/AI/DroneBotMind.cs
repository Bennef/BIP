 ...
 * Outlined Brief: -----------------------------
 * 
-	Traverse on a smaller set path.
-	Will be alerted to the Bip’s presence if they come into the proximity sphere of a drone or if an alarm is sounded by a PatrolBot.
-	Once alerted to Bip’s presence, drones will pursue the player until Bip has been out of their proximity sphere for 5 seconds.
-   When Bip is in the drone proximity sphere, the player will be hit with the drone’s laser attack.
-   Drones will become inactive if a signal jammer is placed inside their proximity sphere.
...
public class DroneBotMind : MindActor 
{
	...
	public override void Destroy()
	{
        ...
        deadDrone.transform.position = this.transform.position;
	...
	void JammingDrone()
	{
		...
		if(false)//condition to be un-jammed
		{
