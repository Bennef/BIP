...
// A general pathing script to be attached to Enemy Objects with PathingPoint game objects.
public class AIPathingScript : MonoBehaviour 
{
	...
	public bool preformingIdleAction = false;
    ...
    // --------------------------------------------------- Methods -----------------------------------------------
    // --------------------------------------------------------------------
    // Use this for setting references
    void Awake()
	{
		...
		case PathingTypes.RandomAround: if(pathPosition.Length > pathRadius.Length) Debug.LogError("Path is missing a radius (SphereCollider)");
			break;
		case PathingTypes.RandomWithin: if(pathPosition.Length > pathArea.Length) Debug.LogError("Path is missing an area (BoxCollider)");
			break;
		}
	}
    // --------------------------------------------------------------------
    ...
    void FixedUpdate() 
	{
		if(!preformingIdleAction)
		{
			...
			else if (transform.rotation.y != Quaternion.LookRotation(nextDestination - transform.position).y)// || transform.rotation.y > Quaternion.LookRotation(nextDestination - transform.position).y + 1.0f)
			{
				// Face Direction.
				transform.rotation =
					//Quaternion.Lerp(transform.rotation, Quaternion.LookRotation (nextDestination - transform.position), Time.deltaTime * 2.0f);
					Quaternion.RotateTowards (transform.rotation, Quaternion.LookRotation (nextDestination - transform.position), Time.deltaTime * 80.0f);
			}
			else
			{
				// Move Position.
    ...
    void ChangeTarget()
	{
		// No longer preforming idle actions
		preformingIdleAction = false;
