...
public class F3DBeam : MonoBehaviour
{
    ...
    public GameObject player;               // Reference to Bip.
    ...
    void Raycast()
    {
        ...
        if (Physics.Raycast(ray, out hitPoint, MaxBeamLength, layerMask, QueryTriggerInteraction.Ignore))
        {
            ...
            {                
                ...
                case F3DFXType.PlasmaBeam:
                   //We have modified the code to include audio
                    ...
                    if (hitPoint.collider.tag != Tags.Moveable)
                    {
                        ApplyForce(1.5f);
                    }
                    break;
                ...
                case F3DFXType.PlasmaBeamHeavy:
                    ...
                    if (hitPoint.collider.name == "Homing Ico 3" && hitPoint.transform.GetComponent<Health>().value > 0)
                    {
                        hitPoint.transform.GetComponent<Health>().TakeDamage(damage);
                    }
                    ApplyForce(1f);
            ...
            if (rayImpact)
                rayImpact.position = hitPoint.point - transform.forward * 0.0f;
        }
